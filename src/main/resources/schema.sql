	...
	CREATE TABLE IF NOT EXISTS `material2` (
	  ...
	  PRIMARY KEY (`mat2Num`),
	  FOREIGN KEY (`reciInputNum`) REFERENCES recipe (`reciNum`) ON DELETE CASCADE ,
	  FOREIGN KEY (`matNum`) REFERENCES material (`matNum`) on delete cascade
	);
	...
	CREATE TABLE IF NOT EXISTS `cart` (
		...
		`buyAmount` int not null,
		 PRIMARY KEY (`cartNum`),
		 FOREIGN KEY (`productName`) REFERENCES product (`productName`) on delete cascade,
		 FOREIGN KEY (`buyer`) REFERENCES member (`id`) on delete cascade
	);
